...
亚马逊佣金 = 售价 × 15%（大部分品类标准佣金率，部分品类不同需查证）
```
...
|------|-----|------------|----------------------|------|----------------------|
| 最优 | $X | Y% | $X÷Y% = $Z | $P | $Z÷$P = **N%** |
| 中等 | ... | ... | ... | ... | ... |
| 保守 | ... | ... | ... | ... | ... |

⛔ **自检红线：ACOS 绝不可能低于 5%**。如果算出 ACOS < 5%，说明公式用错了，立即停下来检查。
⛔ **常见错误**：把 `CPC × 转化率` 当成每单广告花费 → 这是**错的**。转化率是分母（需点击 1÷CVR 次才出 1 单），不是乘数。
⛔ **另一个常见错误**：用 `毛利率 × 50%` 定义 ACOS 目标 → 这不是 ACOS 的计算方式，ACOS 必须从 CPC 和转化率推导。

### D. 净利计算

```
单件广告费 = 售价 × ACOS（用上面算出的实际 ACOS）
单件净利 = 单件毛利 - 单件广告费 - 退货损耗（售价×退货率×0.5）
净利率 = 单件净利 ÷ 售价 × 100%
```
...
报告中投入产出章节必须包含：
1. 成本结构表（每项带金额和备注/数据来源）
2. ACOS 三场景表（最优/中等/保守，**展示完整中间计算步骤**）
3. 盈亏平衡表（保守/中等/乐观三场景）

**采购成本**：可调用 `ali1688_similar_product` 获取1688货源参考价。
**CPC 数据**：从 `keyword_detail` 获取推荐竞价。
**转化率假设**：新品期 8-10%，稳定期 12-15%（无实际数据时的保守假设）。
