---
name: amazon-product-phase9-scale-decision
description: "亚马逊精益开发 Phase 9：产品经过验证和迭代后的规模化决策。评估单位经济学、供应链就绪度、竞争护城河、品牌建设能力，输出 Go Big / Maintain / Harvest / Exit 四象限决策。含 12 个月财务预测模型和产品线扩展规划。优先调用 Sorftime MCP；无 Sorftime 时降级为 WebSearch + 用户手动输入运营数据。"
...
`amazon-product-phase9-scale-decision` 是精益产品开发闭环的最后阶段。产品已经过 MVP 验证（Phase 3）、设计调研（Phase 5）、工业设计（Phase 6）和 1-2 轮迭代优化（Phase 7-8），需要回答一个关键问题：**这个产品值得 all-in 吗？**
...
### 方式1：指定 ASIN
```
/amazon-product-phase9-scale-decision B0XXXXXXXX US --months 6
```

### 方式2：带目标营收
```
/amazon-product-phase9-scale-decision B0XXXXXXXX US --months 4 --target-revenue 50000
```
...
1. **读取完整方法论** — 加载 `skills/amazon-product-phase9-scale-decision/SKILL.md`
...
- 上游：`amazon-product-phase8-iteration-playbook`（Phase 8：迭代优化）+ `amazon-product-phase7-launch-review`（Phase 7：上架复盘）
- 下游：Go Big 决策后进入规模化执行

## 完整方法论

详见 `skills/amazon-product-phase9-scale-decision/SKILL.md`
