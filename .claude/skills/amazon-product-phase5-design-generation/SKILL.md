---
name: amazon-product-phase5-design-generation
description: "亚马逊精益开发 Phase 5：工业设计概念图生成。基于 Phase 4 设计调研成果（品类基本型、形态约束、基准草图、风格规划），自动生成风格概念图、执行偏离度检查、结构化评审、主推方案深化（场景图/正面图/剖面图/全家福）。"
argument-hint: "[产品名称] [--research-pack 调研数据包路径] [--style 风格] [--variants 变体数]"
user-invocable: true
---

# 工业设计概念图生成（精益开发 Phase 5）

## 定位

`amazon-product-phase5-design-generation` 是精益产品开发闭环的第五阶段，衔接上游的设计调研（`amazon-product-phase4-design-research`，Phase 4）。本阶段专注于**概念图生成**——基于 Phase 4 输出的 `design_research_pack.json`（品类基本型描述、形态约束、基准草图、风格规划），自动构建专业 Prompt 并调用图像生成 API，生成高质量工业设计概念图。

**本阶段的所有 Prompt 构建必须基于 Phase 4 的调研成果，不得跳过调研直接生图。**
...
### 方式 1：Slash 命令（关联 Phase 4 调研包）
```
/amazon-product-phase5-design-generation garment steamer --research-pack ./output/design_research_pack.json
```
...
### 方式 3：自定义风格
```
/amazon-product-phase5-design-generation garment steamer --research-pack ./output/design_research_pack.json --style 极简,北欧,赛博朋克,博朗
```
...
| 2 | **加载 Phase 4 调研包** | 读取 `design_research_pack.json`，提取品类基本型描述、形态约束、基准草图、风格规划 |
| 3 | **加载 Phase 3** | 若需补充信息（配色/用户画像/堆叠数据），读取 Phase 3 MVP 蓝图 |
| **4** | **第一轮：风格探索** | 基于调研包的风格规划，构建 4 套 Prompt → 4 张概念图（并行生成）。偏离度目标 **6-15%** |
| **5** | **品类形态偏离度检查** | 将每张生成图与 Phase 4 的**基准草图**对比，评估 8 维度偏离度。>15% 或 <6% 必须重新生成 |
...
执行前必须加载 Phase 4 输出的 `design_research_pack.json`，提取以下必需数据：
...
**如果 `design_research_pack.json` 不存在**，提示用户先执行 Phase 4 设计调研。
...
将每张生成图与 Phase 4 的**基准草图**（`output/images/00_baseline_archetype.jpg`）进行视觉对比。用 Read 工具同时加载生成图和基准草图。

> ⚠️ 注意：对比对象是 Phase 4 的**基准草图**，不是竞品原图。基准草图是统一的品类形态锚点。
...
- 使用姿态：基于 Phase 4 场景形态约束（握持角度/手臂姿态/操作方向）
...
- 半剖结构：左半剖内部 + 右半完整外观
- 所有内部组件从 Phase 3 堆叠数据（经 Phase 4 确认）逐区列出
...
2. **使用基本型** — 基于 Phase 4 品类基本型调研确定的经典外形，不使用复杂异形曲面
3. **保持品类辨识度** — 消费者一眼能认出这是什么产品（通过 Phase 4 品类基本型约束确保）
4. **有且仅有一个记忆特征** — 消费者必须记住一个东西，这个特征同时也是品牌锚点
5. **遵从人体工学** — 使用时手腕保持中性位，不做反关节运动（基于 Phase 4 场景形态约束）
...
2. **Phase 4 调研回顾** — 品类基本型描述摘要 + 形态约束摘要 + 基准草图展示 + 风格规划摘要
...
- 上游：`amazon-product-phase4-design-research`（Phase 4：设计调研 → design_research_pack.json + 基准草图 + 竞品图库）
- 下游：`amazon-product-phase6-launch-review`（Phase 6：上架复盘）
